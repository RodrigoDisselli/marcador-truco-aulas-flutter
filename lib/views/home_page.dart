...
class _HomePageState extends State<HomePage> {
  ...
  void _resetScore(Player player, bool resetVictories){
     // setState() -> faz a tela ser recarregada e executar a modificação
    setState(() {
      player.score = 0;
    });
  }
  ...
  Widget _buildAppBar(){
    return AppBar(
      title: Text('Marcador de truco', style: TextStyle(color: Colors.indigoAccent),),
      backgroundColor: Colors.white,
      actions: <Widget>[
        IconButton(
          icon: Icon(Icons.refresh, color: Colors.indigoAccent,),
          onPressed: () {
